          ...
          <div className="relative glass-card rounded-3xl overflow-hidden border border-brand-200/50 shadow-2xl shadow-brand-200/30">
            ...
            <div className="bg-slate-50 p-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {[
                  { label: 'Toplam Gelir', value: '₺2.4M', change: '+12%', color: 'text-emerald-600' },
                  { label: 'Aktif Siparişler', value: '847', change: '+5%', color: 'text-brand-600' },
                  { label: 'Tedarikçiler', value: '124', change: '+2%', color: 'text-amber-600' },
                  { label: 'Stok Değeri', value: '₺890K', change: '-3%', color: 'text-rose-500' },
                ].map((item, i) => (
                  <div key={i} className="bg-white rounded-xl p-4 border border-slate-100">
                    ...
                    <div className="text-slate-400 text-xs mt-1">{item.change} bu ay</div>
                  </div>
                ))}
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2 bg-white rounded-xl p-4 border border-slate-100">
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-slate-700 text-sm font-medium">Gelir Grafiği</span>
                    <span className="text-brand-500 text-xs">Son 12 ay</span>
                  </div>
                  {/* Fake chart bars */}
                ...
                <div className="bg-white rounded-xl p-4 border border-slate-100">
                  <div className="text-slate-700 text-sm font-medium mb-3">Modüller</div>
                  {[
                    { name: 'Finans', pct: 92, color: '#6366f1' },
                    { name: 'Satış', pct: 78, color: '#059669' },
                    { name: 'Tedarik', pct: 65, color: '#d97706' },
                  ].map((m, i) => (
